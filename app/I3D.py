...
# i3d.load_state_dict(torch.load('/kaggle/working/FINAL_nslt_2000.pt'))
...
# vid_path = '/kaggle/input/wlasl-processed/videos/00335.mp4'
# v_t = data_reader(vid_path)
# v_t = v_t.unsqueeze(0)
#
...
# print(out_labels)
file_path = r'../wlasl_class_list.txt'


# with open(file_path, 'r') as file:
#     lines = file.readlines()
#     print(lines[int(out_labels)])
...
class Predictor:
    ...
    def predicate(self, vid_path):
        v_t = data_reader(vid_path)
        v_t = v_t.unsqueeze(0)

        per_frame_logits = self.i3d(v_t)
        predictions = torch.max(per_frame_logits, dim=2)[0]
        out_labels = torch.argmax(predictions[0]).item()
